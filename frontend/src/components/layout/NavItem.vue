<template>
  <RouterLink
    ...
    :class="isActive ? 'bg-brand-600/20 text-brand-400' : 'text-slate-400 hover:text-white hover:bg-surface-700'"
  >
...
const isActive = computed(() => route.path.startsWith(props.to) && (props.to !== '/' || route.path === '/'))
...
const iconComponent = computed(() =>
  defineComponent({
    render() {
      return h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
